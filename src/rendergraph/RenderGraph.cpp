...
namespace engine::rendergraph {
    ...
    bool RenderGraph::Compile()
    {
        ...
        // WAR (Write-After-Read) detection: must run before CullDeadPasses()
        // because dead-pass culling disables passes that only read (no output),
        // making them invisible to a post-cull check.
        {
            std::unordered_map<RGResourceID, std::string> lastReader;
            for (RGPassID pid : m_sortedPasses)
            {
                ...
                for (const RGResourceAccess& acc : pass.accesses)
                {
                    ...
                    if (acc.IsWrite())
                    {
                        auto it = lastReader.find(acc.resource);
                        if (it != lastReader.end())
                        {
                            const RGResourceDesc& r = m_resources[acc.resource];
                            Debug::LogError(
                                "RenderGraph.cpp: Compile - WAR hazard: Pass '%s' schreibt '%s' "
                                "das bereits von Pass '%s' gelesen wurde",
                                pass.debugName.c_str(), r.debugName.c_str(), it->second.c_str());
                            return false;
                        }
                    }
                    else if (acc.IsRead())
                    {
                        lastReader[acc.resource] = pass.debugName;
                    }
                }
            }
        }
    ...
    void RenderGraph::BuildDependencies()
    {
        ...
        for (auto& res : m_resources)
        {
            ...
            res.firstUsePass = RG_INVALID_PASS;
            res.lastUsePass = RG_INVALID_PASS;
            res.writeGeneration = 0u;
            if (!res.externalOutput)
                res.plannedFinalState = res.plannedInitialState;
        }
    ...
    void RenderGraph::ComputeLifetimes()
    {
        ...
        {
            r.firstUsePass = RG_INVALID_PASS;
            r.lastUsePass = RG_INVALID_PASS;
        }

        for (const RGPassID pid : m_sortedPasses)
        {
            const auto& pass = m_passes[pid];
            if (!pass.enabled)
                continue;
            ...
            for (const auto& acc : pass.accesses)
            {
                ...
                if (r.firstUsePass == RG_INVALID_PASS)
                    r.firstUsePass = pid;
                r.lastUsePass = pid;
            }
        }
    ...
    void RenderGraph::CullDeadPasses()
    {
        ...
        for (auto& pass : m_passes)
        {
            if (pass.enabled && !livePass[pass.id])
            {
                pass.enabled = false;
                ++culledCount;
                Debug::LogVerbose("RenderGraph.cpp: CullDeadPasses - culled '%s'",
    ...
    bool RenderGraph::TopologicalSort()
    {
        ...
        std::vector<std::vector<RGPassID>> outgoing(n);

        for (RGPassID passId = 0u; passId < static_cast<RGPassID>(n); ++passId)
        {
            RGPass& pass = m_passes[passId];
            if (!pass.enabled)
            {
                pass.dependencies.clear();
                continue;
            ...
            for (RGPassID dep : pass.dependencies)
            {
                if (!IsActiveDependency(m_passes, dep) || dep == pass.id)
                    continue;
                ...
                filtered.push_back(dep);
                ++inDegree[pass.id];
                outgoing[dep].push_back(pass.id);
            }
            pass.dependencies = std::move(filtered);
        }
        ...
        while (!queue.empty())
        {
            ...
            for (RGPassID dependent : outgoing[cur])
            {
                if (!m_passes[dependent].enabled)
                    continue;
