#version 410 core
// =============================================================================
// KROM Engine – assets/pbr_lit.frag
...
void main()
{
    ...
    vec4 albedo4 = texture(tAlbedo, vTexCoord) * baseColorFactor;
#else
    ...
    emissive *= texture(tEmissive, vTexCoord).rgb;
#endif
...
#ifdef KROM_ORM_MAP
    vec3 orm = texture(tORM, vTexCoord).rgb;
    ao = mix(1.0, orm.r, clamp(occlusionStrength, 0.0, 1.0));
    roughness = clamp(orm.g * roughnessFactor, 0.0, 1.0);
    metallic = clamp(orm.b * metallicFactor, 0.0, 1.0);
#endif
    roughness = clamp(roughness, 0.04, 1.0);

    vec3 N = SafeNormalize(vNormalWS);
#ifdef KROM_NORMAL_MAP
    N = PerturbNormal(N, vTangentWS.xyz, vTangentWS.w, texture(tNormal, vTexCoord).rgb);
#endif
    ...
    vec3 prefilt = textureLod(tIBLPrefiltered, SphereUV(R), roughness * MAX_REFLECTION_LOD).rgb;
