...
static AABB TransformAABB(const AABB& local, const Mat4& world)
{
    ...
    AABB r{{FLT_MAX,FLT_MAX,FLT_MAX},{-FLT_MAX,-FLT_MAX,-FLT_MAX}};
    for (const auto& c : corners)
    {
        Vec3 p = world.TransformPoint(c);
        r.min.x = std::min(r.min.x, p.x); r.min.y = std::min(r.min.y, p.y); r.min.z = std::min(r.min.z, p.z);
        r.max.x = std::max(r.max.x, p.x); r.max.y = std::max(r.max.y, p.y); r.max.z = std::max(r.max.z, p.z);
    }
    return r;
}
