// =============================================================================
// KROM Engine – assets/pbr_lit.dx11.ps.hlsl
// Pixel Shader: PBR-Lit (DX11 / HLSL SM5)
//
// Farb-/Lichtvertrag:
...
float3 EvalBRDF_GGX(float3 N, float3 V, float3 L, float3 albedo, float metallic, float roughness)
{
...
float4 main(PSInput IN) : SV_Target
{
...
#endif
    return float4(albedo + emissiveColor, opacity);
#endif
    ...
    float3 indirect = (kD_a * albedo * irradiance + specIBL) * ao;
#else
    float3 indirect = kD_a * albedo * ao;
#endif
    ...
    [loop] for (uint i = 0; i < lightCount; ++i)
    {
        ...
        Lo += EvalBRDF_GGX(N, V, L, albedo, metallic, roughness) * (c.rgb * c.a * attenuation);
    }
