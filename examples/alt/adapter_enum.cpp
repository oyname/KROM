...
int main()
{
    ...
    if (!deviceFactoryRegistry.IsRegistered(DeviceFactory::BackendType::DirectX11))
    {
        ...
        DeviceFactory::BackendType::Vulkan })
    {
        ...
        if (!deviceFactoryRegistry.IsRegistered(backend))
        {
            std::cout << name << ": not registered\n\n";
            continue;
        }

        const auto backendAdapters = deviceFactoryRegistry.EnumerateAdapters(backend);
        std::cout << name << ":\n";

        for (const auto& a : backendAdapters)
            std::cout << "  [" << a.index << "] " << a.name
            << "  " << FormatMB(a.dedicatedVRAM)
