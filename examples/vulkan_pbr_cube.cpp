...
int main()
{
    ...
    // Backend
    // -------------------------------------------------------------------------
    if (!renderer::DeviceFactory::IsRegistered(renderer::DeviceFactory::BackendType::Vulkan))
        return -10;

    const auto adapters = renderer::DeviceFactory::EnumerateAdapters(
        renderer::DeviceFactory::BackendType::Vulkan);
    ...
    wDesc.title = "KROM - PBR Rotating Cube (Vulkan)";
    wDesc.width = 1280;
    wDesc.height = 720;
    ...
    if (!loop.Initialize(renderer::DeviceFactory::BackendType::Vulkan,
    ...
    vLayout.attributes.push_back({
        renderer::VertexSemantic::Tangent,
        renderer::Format::RGBA32_FLOAT,
        0u, 24u
        });
    vLayout.attributes.push_back({
    ...
    while (!loop.ShouldExit())
    {
        ...
        {
            Debug::LogError("vulkan_pbr_cube: failed to build render view from ECS camera");
            break;
        }
