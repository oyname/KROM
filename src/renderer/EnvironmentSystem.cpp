...
[[nodiscard]] static Float3 operator+(Float3 a, const Float3& b) noexcept { return a += b; }
[[nodiscard]] static Float3 operator-(const Float3& v) noexcept { return { -v.x, -v.y, -v.z }; }
[[nodiscard]] static Float3 operator-(const Float3& a, const Float3& b) noexcept { return { a.x - b.x, a.y - b.y, a.z - b.z }; }
[[nodiscard]] static Float3 operator*(const Float3& a, float s) noexcept { return { a.x * s, a.y * s, a.z * s }; }
[[nodiscard]] static Float3 operator*(float s, const Float3& a) noexcept { return a * s; }
[[nodiscard]] static Float3 operator/(const Float3& a, float s) noexcept { return { a.x / s, a.y / s, a.z / s }; }
[[nodiscard]] static Float3 operator*(const Float3& a, const Float3& b) noexcept { return { a.x * b.x, a.y * b.y, a.z * b.z }; }
...
[[nodiscard]] static float DistributionGGX(float NoH, float roughness) noexcept
{
...
[[nodiscard]] static uint16_t FloatToHalf(float f) noexcept
{
    uint32_t x = 0u;
    std::memcpy(&x, &f, sizeof(float));
    const uint32_t sign = (x >> 16u) & 0x8000u;
    const int32_t exp = static_cast<int32_t>((x >> 23u) & 0xFFu) - 127 + 15;
    const uint32_t mant = (x >> 13u) & 0x03FFu;
    if (exp <= 0)
