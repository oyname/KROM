...
//   - Shader with COLOR input (triangle_color.vs.hlsl / triangle_color.ps.hlsl)
...
//           Required files: triangle_color.vs.hlsl, triangle_color.ps.hlsl,
//                           fullscreen.vs.hlsl, passthrough.ps.hlsl
...
int main()
{
    ...
    const ShaderHandle vs = pipeline.LoadShader("triangle_color.vs.hlsl",
        assets::ShaderStage::Vertex);
    const ShaderHandle ps = pipeline.LoadShader("triangle_color.ps.hlsl",
        assets::ShaderStage::Fragment);

    // Tonemap passthrough shaders (required by the forward feature)
    const ShaderHandle tvs = pipeline.LoadShader("fullscreen.vs.hlsl",
        assets::ShaderStage::Vertex);
    const ShaderHandle tps = pipeline.LoadShader("passthrough.ps.hlsl",
        assets::ShaderStage::Fragment);
    ...
    mat.renderPass     = renderer::StandardRenderPasses::Opaque();
    mat.vertexShader   = vs;
    ...
    tmDesc.renderPass     = renderer::StandardRenderPasses::Opaque();
