...
void main()
{
    ...
    vec4 albedo4 = texture(uAlbedo, vTexCoord) * baseColorFactor;
#else
    ...
    for (uint i = 0u; i < lightCount; ++i)
    {
        ...
        else
        {
            ...
            if (lightType > 1.5)
                attenuation *= CalcSpotAttenuation(L, normalize(lights[i].directionWS.xyz), lights[i].params.x, lights[i].params.y);
        }
